...
const SearchByBoard = () =>  {
  ...
  const fetchData = (board) => {
    
    fetch(`http://192.168.1.2:3000/searchSchoolsByBoard?board=${board}`)
      .then(response => response.json())
      .then(jsonResponse => setSchool(jsonResponse))
      .catch(error => console.log(error))
  };
  useEffect(() => {
    const board = route.params?.board; // Access the name from the route.params object
    fetchData(board);
  }, [route.params]); 
    const navigation= useNavigation();
  ...
  return (
    ...
    <Image source={require('./assets/connectschoolslogo.png')} style={{width:215,height:72}} />
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center',paddingLeft:5,paddingRight:80,paddingTop:12,justifyContent:'space-between'}}>
        <Text style={{color:"#101C6C", fontSize: 20}}>Schools for {route.params?.board} board</Text>
